...
from core.solve_captcha import solve_image_captcha
...
def activity(profile_number, wallet_addr, email=None, email_password=None):
    try:
        ...
        with sync_playwright() as playwright:
            ...
            for _ in range(10):
                if "channels" in page.evaluate("location.href"):
                    break
                time.sleep(10)
            if page.get_by_text("Builder").count() == 1:
                page.get_by_text("Builder").click()
            page.get_by_text("Finish").click()
            page.get_by_text("Verify yourself").click()
            page.get_by_role("button", name="Verify").click()
            page.get_by_role("button", name="Continue").click()
            page.get_by_role("button", name="Continue").click()
            time.sleep(4)
            src = page.locator('a[href*="captcha"]').get_attribute("href")
            code = solve_image_captcha(src, 6, numbers=True)
            for i in list(code):
                page.get_by_role("button", name=i, exact=True).click()
                time.sleep(4)
            time.sleep(3)
