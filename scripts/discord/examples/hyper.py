...
def activity(profile_number, wallet_addr, email=None, email_password=None):
    try:
        ...
        with sync_playwright() as playwright:
            ...
            page.get_by_text("Accept Invite").click()

            while "channels" not in page.evaluate("location.href"):
                time.sleep(5)
            page.get_by_role("button", name="Complete").last.click()
            page.get_by_text("I have").click()
            page.get_by_role("button", name="Submit").click()

            page.get_by_role("button", name="Start Verification").wait_for(timeout=60000)
            page.get_by_role("button", name="Start Verification").click()

            time.sleep(2)
            word = page.locator('[class*="embedDescription"] strong').last.inner_text().strip()
            page.get_by_text("Please select the correct option").click()
            # target = word.lower().replace(" ","").replace("-","")
            # for option in page.get_by_role("option").all():
            #     item_id = option.get_attribute("data-list-item-id") or ""
            #     suffix = item_id.split("___")[-1].lower()
            #     if suffix == target:
            #         option.click()
            #         break

            for option in page.get_by_role("option").all():
                emoji = option.locator("img.emoji").get_attribute("alt") or ""
                if ask(f"Does '{emoji}' represent '{word}'? Reply YES or NO.") == "YES":
                    option.click()
                    break
