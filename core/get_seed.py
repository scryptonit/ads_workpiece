...
def get_seed_for_address(address: str):
    wallets = get_wallets()
    seed_map = {addr.lower(): seed for addr, _, seed in wallets}

    seed = seed_map.get(address.lower())
    if seed is None:
        raise KeyError(f"No seed found for address '{address}'")
    return seed
